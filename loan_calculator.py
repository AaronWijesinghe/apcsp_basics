...
def get_loan_info():
    ...
    if loan["monthly payment"] >= loan["principal"]:
        print("You already have enough money! You don't need a loan!")
        raise SystemExit
...
def collect_interest(loan, principal_type="current"):
    # Add interest to the loan, according to the starting/current principal
    if principal_type == "current":
        loan["principal"] += loan["principal"] * loan["interest"] / 12
    elif principal_type == "starting":
        loan["principal"] += starting_principal * loan["interest"] / 12
